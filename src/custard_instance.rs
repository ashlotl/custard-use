	...
	pub(crate) fn partial_reload(mut self, new_unloaded_composition: UnloadedComposition, checked: Checked, reload_for_sure: Arc<BTreeSet<CrateName>>) {
		...
		for (crate_name, old_crate) in unsafe { old_composition.crates.get_mut() } {
			...
			if !(!reload && old_unloaded_crate == new_unloaded_crate) {
				let delta_tasks = new_unloaded_crate.as_ref().unwrap().tasks.len() - old_unloaded_crate.as_ref().unwrap().tasks.len();
				old_composition.task_completion.begin_fulfillers(delta_tasks as isize);
				continue;
			}
